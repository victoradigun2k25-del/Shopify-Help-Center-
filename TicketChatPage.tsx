...
const TicketChatPage: React.FC<TicketChatPageProps> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    // Live refresh every 2 seconds for new messages
      ...
      try {
        ...
        if (fresh) {
          setTicket((prev) => {
            ...
            if (!prev || fresh.messages.length !== prev.messages.length) return fresh;
            return prev;
          });
  ...
  /** Handle sending messages smoothly */
  const handleSendMessage = async (text: string, type: 'text' | 'image' | 'file') => {
    ...
    // Instant update locally (no flicker)
    ...
    return (
      <div className="max-w-2xl mx-auto mt-20 p-8 bg-white rounded-2xl border border-gray-200 shadow-sm text-center">
        ...
        <p className="text-gray-600 mb-8">
          {error || "The ticket you're looking for doesn't exist or you don't have access."}
        </p>
        <Link
          to="/"
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            <Link
              to="/"
              className="p-2 bg-white rounded-lg border border-gray-200 text-gray-500 hover:text-emerald-600 transition-colors shadow-sm"
            >
              <ArrowLeft size={20} />
            </Link>
            <div>
              ...
              <p className="text-sm text-gray-500">
                Subject: Order Assistance & General Support
              </p>
            </div>
          </div>
        ...
        {/* Chat Section */}
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden h-[70vh] sm:h-[700px] flex flex-col">
          <ChatWindow
            ticket={ticket}
          ...
          />
          {sending && (
            <div className="text-center text-xs py-1 text-gray-400 bg-gray-50">
              Sending message...
            </div>
          )}
        </div>
        ...
        <div className="mt-6 p-4 bg-gray-100 rounded-xl text-center text-xs text-gray-500">
          Please note: Support representatives will never ask for your password or credit card
          details via chat.
        </div>
      </div>
    </div>
